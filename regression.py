# -*- coding: utf-8 -*-
"""
Created on Tue Dec  8 20:18:47 2020
...
# survival_df_filtered.to_csv('survial_df_filtered.csv')

# Read in features
t1_features = pd.read_csv('features/t1_radio_features.csv', header=None)
t1Gd_features = pd.read_csv('features/t1Gd_radio_features.csv', header=None)
t2_features = pd.read_csv('features/t2_radio_features.csv', header=None)
flair_features = pd.read_csv('features/flair_radio_features.csv', header=None)
...
haha = all_features_reduced.drop(['original_glrlm_GrayLevelNonUniformityNormalized_1'],axis=1)
haha=haha.drop(['original_glcm_SumEntropy_1'],axis=1)

my_cph.fit(haha, duration_col = 'days_to_death', event_col='vital_status')
my_cph.print_summary()

scores = k_fold_cross_validation( my_cph,all_features_reduced, duration_col = 'days_to_death', event_col='vital_status', k=10,
